...
//            button.frame = CGRectMake(CONTENT_PADDING_LEFT, y, width, BUTTON_HEIGHT);
            button.frame = CGRectMake(CONTENT_PADDING_LEFT, y + 10, width, BUTTON_HEIGHT);
            button = self.buttons[1];
//            button.frame = CGRectMake(CONTENT_PADDING_LEFT + width + GAP, y, width, BUTTON_HEIGHT);
            button.frame = CGRectMake(CONTENT_PADDING_LEFT + width + GAP, y + 10, width, BUTTON_HEIGHT);
            // 吴超修改
            UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0.5, 44)];
            [view setBackgroundColor:UIColorFromHex(0xc1c1c1)];
            [button addSubview:view];
            UIView *buttonView = [[UIView alloc] initWithFrame:CGRectMake(-150, 0, 300, 0.5)];
            [buttonView setBackgroundColor:UIColorFromHex(0xc1c1c1)];
            [button addSubview:buttonView];

        } else {
            for (NSUInteger i = 0; i < self.buttons.count; i++) {
                UIButton *button = self.buttons[i];
                button.frame = CGRectMake(CONTENT_PADDING_LEFT, y + 10, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, BUTTON_HEIGHT);
                // 吴超修改
                UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, button.frame.size.width, 1)];
                [view setBackgroundColor:UIColorFromHex(0xc1c1c1)];
                [button addSubview:view];
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
	...
	switch (item.type) {
		...
		case SIAlertViewButtonTypeDestructive:
			...
			break;
        case SIAlertViewButtonTypePeng:
            [button setBackgroundColor:[UIColor clearColor]];
            [button setTitleColor:self.buttonColor ? self.buttonColor : UIColorFromHex(0x08a9ec) forState:UIControlStateNormal];
//            [button setTitleColor:[UIColor colorWithWhite:1 alpha:0.8] forState:UIControlStateHighlighted];
            break;
...
- (void)setTitleColor:(UIColor *)titleColor
{
    ...
    self.titleLabel.textColor = titleColor ? titleColor : UIColorFromHex(0x101010);
}
