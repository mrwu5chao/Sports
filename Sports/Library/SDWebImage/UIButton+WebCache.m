...
- (void)setDefaultImageWithURL:(NSURL *)url forState:(UIControlState)state {
    [self setDefaultImageWithURL:url forState:state placeholderImage:[UIImage imageNamed:@"default_image"]];
}

- (void)setDefaultImageWithURL:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder {
    [self cancelCurrentImageLoad];
    
    [self setImage:placeholder forState:state];
    
    UIColor *oldColor = self.backgroundColor;
    [self setBackgroundColor:UIColorFromHex(0xdddddd)];
        ...
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadWithURL:url options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
            ...
            dispatch_main_sync_safe(^{
                ...
                if (image) {
                    ...
                    [self.imageView setContentMode:oldMode];
    ...
    if (url) {
        ...
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadWithURL:url options:options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
            ...
            dispatch_main_sync_safe(^{
                ...
                if (!sself) return;
                if (image) {
                    if ([self.backgroundColor isEqual:UIColorFromHex(0xdddddd)]) {
